...
from packages.output import write_binary, write_csv
...
def run():
    ...
    output_demod_file = os.path.join(curr_dir, "output/demodulated.csv")
    demodulated_sequence = demodulate(data_seq, channel)
    write_csv(output_demod_file, demodulated_sequence)
